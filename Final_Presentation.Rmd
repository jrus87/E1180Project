...
- Concentration of crime in one area. It seems that retailers not associated with crime
...
- Looks like coding/data processing error in January 2014, but later no statistically significant difference
- Development varies for each category -> natural logarithm (i.e. percentage change month-to-month) on next slide
...
- We see Property, Burglary/Theft and Other Drug Related change substantially
```{r, echo = FALSE, message = FALSE, warning = FALSE}
crime.graph.log
```
...
- We can see there is a significant relationship between the establishment and crime development, but once we match similar districts, this drops substantially\vspace{12pt}
- Additionally, the R? becomes much lower, highlighting that the variance explained by the establishment is rather low\vspace{12pt}
- An explanation for this could be that crime is spatially concentrated and auto-correlated with previous crime levels\vspace{12pt}
- Retailers might choose not to start up near crime areas (i.e. self-selection bias)

## Conclusion (II)

- Once accounted for GEOID: the point estimation are smaller -> GEOID important\vspace{12pt}
- Once accounted for Month: the point estimation are higher -> maybe due to the geoid issue (time-trend not as important as spatial?)\vspace{12pt}
